...
//             <img src={`/assets/avatars/${profileData.logo}`} alt="Profile" className="w-full h-full object-cover" />
//           </div>
...
export default function SocialProfile () {
  const {id} = useParams();
  const {data: socialGroup} = useFetch(`http://localhost:5000/api/socialgroup/${id}`);
  const {data: campaigns} = useFetch(`http://localhost:5000/api/campaigns/socialgroup/${id}`);
  return (
    <div className='min-h-screen lg:ml-[245px] p-4'> 
      <div className='min-h-screen p-8 rounded-pl bg-neutral'>
        <div className="w-full">
          <img src={`/assets/${socialGroup.banner}`} className="rounded-pl object-cover w-full h-60" />
        </div>
        <img src={`/assets/avatars/${socialGroup.image}`} className="object-cover rounded-full w-40 h-40 ml-6 -mt-12" />
        <h1 className="font-bold mt-4">{socialGroup.name}</h1>
        <p>{socialGroup.initiative}</p>
        <p className="mt-4">Location: {socialGroup.location}</p>
        {/* <p>Rating:{socialGroup.rating}</p> */}
        <div className="grid grid-cols-1 lg:grid-cols-4">
          {/* <div className="grid grid-cols-1 lg:grid-cols-4 gap-4"> 
            {campaigns.map((campaign, index) => ( 
              <CampaignCardSh key={index} campaign={campaign} />
            ))} 
          </div> */}
        </div>
      </div>
