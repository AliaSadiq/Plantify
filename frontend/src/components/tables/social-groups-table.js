...
export default function SocialGroupTable({ socialGroups, setSocialGroups }) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this group?")) {
            try {
                await axios.delete(`http://localhost:5000/api/socialgroup/${id}`);
                // Remove the deleted group from the state
                setSocialGroups((prevGroups) => prevGroups.filter((group) => group._id !== id));
            } catch (error) {
                console.error("Error deleting the social group:", error);
            }
        }
    };
