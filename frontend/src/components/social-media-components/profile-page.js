...
const ProfilePage = ({ onClose }) => {
  ...
  const userId = useFetchUserLocalStorage();
  const profileFromModal = location.state?.profileData;
  const currentUserId = userId?._id; // Logged-in user's ID
  const currentId = profileFromModal ? profileFromModal._id : currentUserId;

  const { profile, loading: profileLoading, error: profileError } = useProfile(currentId);
  const { toggleFollow, loading: followLoading, error: followError } = useFollowUser();
  const [isFollowing, setIsFollowing] = useState(false);

  // Determine if the profile belongs to the logged-in user
  const isOwnProfile = currentId === currentUserId;
    ...
    <div className="w-full p-4 flex flex-col items-start">
      <div className="flex justify-between items-center mb-6 w-full">
        <div className="flex items-center">
          <div className="w-20 h-20 bg-gray-200 rounded-full overflow-hidden border-4 border-gradient-to-r from-yellow-400 to-red-500 flex items-center justify-center mr-4">
            <img src={`/assets/${avatar}`} alt={name} className="object-cover w-full h-full" />
          </div>
          <div>
            <h2 className="text-xl font-semibold">{name}</h2>
        ...
        {!isOwnProfile && ( // Hide the button for own profile
          <button
            onClick={handleFollow}
            className={`px-4 py-2 rounded-lg font-medium ${
              followLoading ? 'bg-gray-400' : isFollowing ? 'bg-red-500 text-white' : 'bg-blue-500 text-white'
