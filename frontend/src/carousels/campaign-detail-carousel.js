...
export default function CampaignDetailsCarousel({ campaign }) {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/campaign-comment/${campaign._id}`);
                    ...
                    <div className="flex flex-row gap-2 items-center justify-center">
                        <div className="overflow-y-auto max-h-60 font-josefin-sans p-6 mx-10">
                            {comments.length > 0 ? comments.map((comment) => {
                                ...
                                return (
                                    <div key={comment._id} className="relative grid grid-cols-1 gap-4 p-4 mb-8 bg-navygreen-100 shadow-lg">
                                        <div className="relative flex gap-4">
                                            <img src={avatar} className="relative rounded-lg -top-[25px] -mb-4 bg-navygreen-100 h-16 w-16" alt="" loading="lazy" />
                                            <div className="flex flex-col w-full">
                                                <div className="flex flex-row justify-between">
                                                    <p className="relative text-mini whitespace-nowrap truncate overflow-hidden">
                                                        {comment.user && comment.user.username ? comment.user.username : 'Anonymous'}
                                                    </p>
                                                    <a className="text-gray-500 text-sm" href="#">{comment.amount}</a>
