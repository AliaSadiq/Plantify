...
import useAllCampaigns from '../hooks/useAllCampaigns'; // New hook for fetching all campaigns
...
const CampaignPage = () => {
  ...
  const { campaigns, loading, error, totalPages } = useCampaigns(currentPage, 6); // Fetch campaigns with pagination
  const { allCampaigns, loading: allLoading, error: allError } = useAllCampaigns(); // Fetch all campaigns
  const { recentCampaigns } = useRecentCampaigns(); // Fetch recent campaigns
  ...
  return (
    <div>
      {/* Carousel */}
      <CampaignHeaderCarousel campaigns={campaigns} onSearch={handleSearch} />
      <div className="my-10 gap-60 flex item-center justify-center">
        <p className="text-xl font-bold text-gray-100 font-josefin-sans">
          List of all the campaigns
        </p>
        <div className="">
          <FilterDropdown /> {/* No backend functionality, left as is */}
        </div>
      </div>
      <div className="flex px-20 gap-8 justify-between">
        <div className="flex flex-col pl-4 py-4 bg-navygreen-100 rounded-pl items-start w-[230px] max-h-fit">
          <h2 className="mt-2 font-bold text-md ml-2">Categories</h2>
          <ul className="flex flex-col gap-y-2 rounded-lg mt-4 text-sm">
            <li className="p-2 hover:bg-navygreen-200 rounded-lg hover:font-semibold">Donation Campaigns</li>
            <li className="p-2 hover:bg-navygreen-200 rounded-lg hover:font-semibold">Awareness Campaigns</li>
        ...
        <div>
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:gap-4 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
              {(filteredCampaigns.length > 0 ? filteredCampaigns : campaigns).map(campaign => (
                <CampaignCardSh key={campaign._id} campaign={campaign}/>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-1">
            {recentCampaigns.map(campaign => (
              <div key={campaign._id} className="mt-4 flex flex-row gap-4">
                ...
                <div className="self-center flex flex-col">
                  <h3 className="text-sm font-semibold">{campaign.name}</h3>
                  <p className="text-sm font-josefin-sans text-gray-500">
